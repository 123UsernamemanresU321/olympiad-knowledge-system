...
export function TopicPage() {
  ...
  const rows = topicEntries.map((entry) => ({
    id: entry.id,
    title: entry.title,
    route: entry.route,
      ...
      <div className="mx-auto flex w-full max-w-[1320px] flex-col gap-8 px-6 py-10">
        ...
        <div className="flex flex-col gap-8 border-b border-base-600 pb-8 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link
              to={isAdmin ? '/import' : '/login?redirect=/import'}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-[8px] bg-primary-500 px-6 text-sm font-bold text-white shadow-[0_10px_20px_rgba(20,75,184,0.2)] transition-colors hover:bg-primary-400"
            >
        ...
        <Surface className="overflow-hidden">
          {rows.length > 0 ? (
            ...
            <div className="p-6 text-sm text-text-500">No authored entries are linked to this topic yet.</div>
          )}
        </Surface>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Surface className="p-5">
            <div className="text-xs uppercase tracking-[0.16em] text-text-500">Subtopics</div>
            <div className="mt-3 text-3xl font-black text-text-100">{resolved.topic.subtopics.length}</div>
            <p className="mt-2 text-sm text-text-400">Taxonomy branches currently attached to this learning lane.</p>
          </Surface>
          <Surface className="p-5">
